...
CLIP = 1e3  # cap daily returns to avoid absurd magnitudes
...
def _safe_daily_return(values: pd.Series) -> pd.Series:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ret = values / prev - 1.0
    # invalid when prev<=0 or ret non-finite → 0
...
def compute_portfolio_kpis(positions_daily, prices):
    ...
    for i in range(len(merged)):
        ...
        alphas.append(float(alpha_daily * 252.0))              # annualize alpha
        te.append(float(residuals.std(ddof=1) * np.sqrt(252))) # tracking error (annualized)
...
if __name__ == "__main__":
    print("USING NUMPY VERSION (no sklearn)")
